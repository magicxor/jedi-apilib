...
uses SysUtils, Contnrs, Classes, System.Types,
  JwaWindows,
...
type
  ...
  protected
    ...
    procedure GetTokenInformation(hTokenHandle: TJwTokenHandle;
      TokenInformationClass: TTokenInformationClass; out TokenInformation: Pointer); virtual;
    ...
    procedure SetTokenDefaultDacl(const aDefaultDCAL: TJwDAccessControlList); virtual;
    // TOKEN_ADJUST_DEFAULT

    function GetTokenOrigin: TLuid; virtual;
    procedure SetTokenOrigin(const anOrigin: TLuid); virtual; // SE_TCB_NAME
    ...
    function GetTokenOwner: TJwSecurityId; virtual;
    {
      Obsolete
      Use the property TokenOwner instead }
    procedure SetTokenOwner(const anOwner: TJwSecurityId); virtual; // TOKEN_ADJUST_DEFAULT

    function GetPrimaryGroup: TJwSecurityId; virtual;
    procedure SetPrimaryGroup(const PrimGroup: TJwSecurityId); virtual; // TOKEN_ADJUST_DEFAULT

    function GetTokenSessionId: cardinal; virtual;
    procedure SetTokenSessionId(const SessionID: cardinal); virtual; // SE_TCB_NAME
  ...
  public
    ...
    procedure GetTokenSource(out SourceName: ShortString; out SourceLUID: TLuid); overload; virtual;
  ...
  public
    ...
    constructor CreateTokenByThread(const aThreadHandle: TJwThreadHandle;
      const aDesiredAccess: TJwAccessMask; const anOpenAsSelf: boolean); virtual;
    ...
    constructor CreateDuplicateExistingToken(const aTokenHandle: TJwTokenHandle;
      const aDesiredAccess: TJwAccessMask; UseDuplicateExistingToken: boolean = false);
      overload; virtual;
    ...
    constructor CreateDuplicateExistingToken(const Token: TJwSecurityToken;
      const DesiredAccess: TJwAccessMask; UseDuplicateExistingToken: boolean = false);
      overload; virtual;
    ...
    constructor Create(const aTokenHandle: TJwTokenHandle; const ShareTokenHandle: TJwSharedHandle;
    ...
    constructor CreateWTSQueryUserTokenEx(const Server: TObject; SessionID: cardinal);
      overload; virtual;
    ...
    constructor CreateCompatibilityQueryUserToken(const DesiredAccess: TJwAccessMask;
    ...
    *)
    constructor CreateNewToken(const aDesiredAccess: TJwAccessMask;
      ...
      const anExpirationTime: int64; anUser: TJwSecurityId; aGroups: TJwSecurityIdList;
      aPrivileges: TJwPrivilegeSet; anOwner: TJwSecurityId; aPrimaryGroup: TJwSecurityId;
      aDefaultDACL: TJwDAccessControlList; aTokenSource: TTokenSource); virtual;
    ...
    class function Create_OBJECT_ATTRIBUTES(const aRootDirectory: THandle;
      ...
      const aContextTrackingMode: SECURITY_CONTEXT_TRACKING_MODE; const anEffectiveOnly: boolean)
      : TObjectAttributes; virtual;

    { <B>Free_OBJECT_ATTRIBUTES</B> removes memory allocated by the members which were created
      by Create_OBJECT_ATTRIBUTES }
    class procedure Free_OBJECT_ATTRIBUTES(anObjectAttributes: TObjectAttributes); virtual;
    ...
    constructor CreateRestrictedToken(PrevTokenHandle: TJwTokenHandle;
      const TokenAccessMask: TJwTokenAccessMask; const Flags: cardinal;
      const SidsToDisable: TJwSecurityIdList; const PrivilegesToDelete: TJwPrivilegeSet;
  ...
  public
    ...
    procedure ConvertToImpersonatedToken(impLevel: SECURITY_IMPERSONATION_LEVEL;
    ...
    function CreateDuplicateToken(AccessMask: TJwAccessMask; Security: PSECURITY_ATTRIBUTES)
      : TJwSecurityToken;
    ...
    function CreateRestrictedToken(const TokenAccessMask: TJwTokenHandle; const Flags: cardinal;
      const SidsToDisable: TJwSecurityIdList; const PrivilegesToDelete: TJwPrivilegeSet;
      const RestrictedSids: TJwSecurityIdList): TJwSecurityToken; overload; virtual;
    ...
    class function PrivilegeCheck(const ClientToken: TJwSecurityToken;
      const RequiredPrivileges: TJwPrivilegeSet; const RequiresAllPrivs: TJwPrivCheck)
      : boolean; overload;
  ...
  public
    ...
    class procedure PrivilegedServiceAuditAlarm(SubsystemName, ServiceName: TJwString;
      ClientToken: TJwSecurityToken; Privileges: TJwPrivilegeSet; AccessGranted: boolean);

    // see equivalent msdn function for more information
    class procedure ImpersonateAnonymousToken(const Thread: TJwThreadHandle); virtual;
    // see equivalent msdn function for more information
    class procedure ImpersonateSelf(const anImpersonationLevel
      : SECURITY_IMPERSONATION_LEVEL); virtual;
    { see equivalent msdn function for more information
    ...
    class function GetThreadToken(const aDesiredAccess: TJwAccessMask; const anOpenAsSelf: boolean)
      : TJwSecurityToken; virtual;
  ...
  public
    ...
    function GetSecurityDescriptor(const SecurityFlags: TJwSecurityInformationFlagSet)
    ...
    procedure SetSecurityDescriptor(const SecurityFlags: TJwSecurityInformationFlagSet;
  ...
  public
    ...
    class procedure SaferComputeTokenFromLevel(const SaferLevelHandle: TSaferLevelHandle;
      const InAccessToken: TJwSecurityToken; SaferFlags: DWORD; out Result: TJwSecurityToken);
      overload; virtual;
  public
  ...
  public
    ...
    property ImpersonationLevel: TSecurityImpersonationLevel Read GetImpersonationLevel;
    ...
    property IsTokenMemberShip[aSID: TJwSecurityId]: boolean Read CheckTokenMembership;
    ...
    property TokenGroups: TJwSecurityIdList Read GetTokenGroups Write SetTokenGroups;
    ...
    property TokenRestrictedSids: TJwSecurityIdList Read GetTokenRestrictedSids;
    ...
    property TokenDefaultDacl: TJwDAccessControlList Read GetTokenDefaultDacl
      Write SetTokenDefaultDacl; // TOKEN_ADJUST_DEFAULT
    ...
    { <B>PrimaryGroup</B> sets or gets the primary group.
      To set the value the token needs TOKEN_ADJUST_DEFAULT privilege }
    property PrimaryGroup: TJwSecurityId Read GetPrimaryGroup Write SetPrimaryGroup;
    // TOKEN_ADJUST_DEFAULT
    ...
    property TokenSessionId: cardinal Read GetTokenSessionId Write SetTokenSessionId;
    ...
    property PrivilegeEnabled[Name: TJwString]: boolean Read GetPrivilegeEnabled
      Write SetPrivilegeEnabled;
    ...
    property PrivilegeAvailable[Name: TJwString]: boolean Read GetPrivilegeAvailable;
    ...
    property TokenIntegrityLevelType: TJwIntegrityLabelType Read GetIntegrityLevelType
      Write SetIntegrityLevelType;
  ...
  public
    ...
    constructor Create(anOwner: TJwPrivilegeSet; Name: String; Attributes: cardinal); overload;
    ...
    class function PrivilegeAttributeToText(PrivilegeAttributes: cardinal;
      HumanReadable: boolean = false): TJwString; virtual;
    ...
    class function MakeLUID_AND_ATTRIBUTES(const LowPart: cardinal; const HighPart: LONG;
    ...
    class function MakeLUID_AND_ATTRIBUTES(const LUID: TLuid; Attributes: cardinal)
      : TLuidAndAttributes; overload;

    { <B>MakeLUID</B> creates a luid strcture from the given parameter. }
    class function MakeLUID(const LowPart: cardinal; const HighPart: LONG): TLuid;
  ...
  public
    ...
    property Privilege_Used_For_Access: boolean Read fPrivilege_Used_For_Access
      Write fPrivilege_Used_For_Access;
  ...
  public
    { <B>Create</B> creates a new instance with a list of privileges.
      ...
      raises
      EJwsclInvalidOwnerException:  will be raised if anOwner is nil. }
    constructor Create(Owner: TJwSecurityToken; Privileges: JwaWindows.TPrivilegeSet); overload;
    { <B>Create</B> creates a new instance with a list of privileges.
      @param Owner contains the owner token of this privilege set. It must not be nil!
    ...
    { <B>Create</B> creates a new instance with a list of privileges from a
      ...
      raises
      EJwsclInvalidOwnerException:  will be raised if anOwner is nil. }
    constructor Create(Owner: TJwSecurityToken; PrivilegeObject: TJwPrivilegeSet); overload;
    ...
    function Create_PTOKEN_PRIVILEGES: JwaWindows.PTOKEN_PRIVILEGES; virtual;
    ...
    procedure Free_PLUID_AND_ATTRIBUTES(var Privileges: PLUID_AND_ATTRIBUTES); virtual;
    ...
    function AddPrivilege(LuidAttributes: LUID_AND_ATTRIBUTES): Integer; overload; virtual;
    ...
    function AddPrivilege(HighValue, LowValue: cardinal): Integer; overload; virtual;
...
function JwGetPrivilegesText(const DisplayPrivileges: array of TJwString): TJwString; overload;
...
function JwCreateRestrictedToken(const DenyUserSIDs: TJwSecurityIdList = nil;
  const RestrictedUserSIDs: TJwSecurityIdList = nil; const RestrictAllSIDs: boolean = false;
...
function JwCreateRestrictedToken(const DenyUserSIDs: TJwSecurityIdList = nil;
  ...
  begin
    ...
    try
      ...
      SD.DACL.Add(TJwDiscretionaryAccessControlEntryAllow.Create(nil, [], STANDARD_RIGHTS_ALL,
...
var
  ...
  SIDList, DenySIDList, RestrictedSIDList: TJwSecurityIdList;
...
begin
  ...
  try
    ...
    else if Assigned(RestrictedUserSIDs) then
    begin
    ...
    if Assigned(DisableUserPrivileges) and (Flags and DISABLE_MAX_PRIVILEGE = 0) then
    begin
    ...
    try
      Result := TJwSecurityToken.CreateRestrictedToken(AdminToken.TokenHandle, TOKEN_ALL_ACCESS,
        Flags, DenySIDList, nil, RestrictedSIDList);
    ...
    try
      ...
      SD.DACL.Add(TJwDiscretionaryAccessControlEntryAllow.Create(nil, [], GENERIC_ALL,
        JwSecurityProcessUserSID, True));
...
function JwIsUACEnabled: boolean;
  ...
  begin
    ...
    if RegOpenKeyExW(HKEY_LOCAL_MACHINE,
      'Software\Microsoft\Windows\CurrentVersion\Policies\System', 0, KEY_READ, Key)
      = HRESULT(ERROR_SUCCESS) then
    begin
      ...
      raise EJwsclWinCallFailedException.CreateFmtWinCall(RsWinCallFailed, 'JwIsUACEnabled',
        // sSourceProc
        '', // sSourceClass
...
begin
  Result := (TJwWindowsVersion.IsWindowsVista(True) or TJwWindowsVersion.IsWindows2008(True))
    and IsLUA;
end;
...
var
  Token: TJwSecurityToken;
begin
  JwCheckInitKnownSid([JwAdministratorsSID], ['JwAdministratorsSID'],
    'JwIsMemberOfAdministratorsGroup', '', RsUNToken);
...
function JwGetPrivilegesText(const DisplayPrivileges: array of TJwString): TJwString;
var
  T: TJwSecurityToken;
  Priv: TJwPrivilege;
  s: TJwPrivilegeSet;
...
begin
  ...
  T := TJwSecurityToken.CreateTokenEffective(TOKEN_DUPLICATE or TOKEN_READ or
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Result := Result + #13#10 + TJwString(DisplayPrivileges[I]) + ' ' + RsTokenUnavailableText;
    end;
...
begin
  ...
  begin
    if (Query = pst_Enable) then
      raise EJwsclPrivilegeException.CreateFmtEx(RsTokenPrivilegeNotAvailable, 'JwEnablePrivilege',
  ...
  T := TJwSecurityToken.CreateTokenEffective(TOKEN_DUPLICATE or TOKEN_READ or
    TOKEN_ADJUST_PRIVILEGES);
  try
    T.PrivilegeEnabled[Index] := (Query = pst_Enable) or (Query = pst_EnableIfAvail);
  finally
    T.Free;
  end;
end;
...
var
  T: TJwSecurityToken;
begin
  T := TJwSecurityToken.CreateTokenEffective(TOKEN_DUPLICATE or TOKEN_READ or TOKEN_QUERY or
    TOKEN_WRITE);
...
constructor TJwPrivilegeSet.Create(Owner: TJwSecurityToken; PrivilegesPointer: PTOKEN_PRIVILEGES);
var
  I: Integer;
begin
...
constructor TJwPrivilegeSet.Create(Owner: TJwSecurityToken; PrivilegeObject: TJwPrivilegeSet);
...
constructor TJwPrivilegeSet.Create(Owner: TJwSecurityToken; Privileges: JwaWindows.TPrivilegeSet);
...
begin
  if (Index >= Count) then
    raise EJwsclInvalidIndexPrivilegeException.CreateFmtEx(RsTokenInvalidPrivilegeIndex,
      'GetPrivByIdx', ClassName, RsUNToken, 0, false, [Index, Count]);
...
begin
  ...
  begin
    p := TJwPrivilege(fList.Items[I]);
    if (Index.LowPart = p.LUID.LowPart) and (Index.HighPart = p.LUID.HighPart) then
    begin
      Result := p;
      exit;
    end;
...
begin
  if not Assigned(Owner) then
    raise EJwsclNotImplementedException.CreateFmtEx(RsTokenRemovePrivilegeDenied,
      'RemoveIrrepealable', ClassName, RsUNToken, 0, false, []);

  Owner.CheckTokenHandle('RemoveIrrepealable');
  Owner.CheckTokenAccessType(TOKEN_QUERY + TOKEN_ADJUST_PRIVILEGES,
    'TOKEN_QUERY,TOKEN_ADJUST_PRIVILEGES', 'TJwPrivilegeSet.RemoveIrrepealable');

  if not Assigned(PrivByName[SE_TCB_NAME]) then
    raise EJwsclAdjustPrivilegeException.CreateFmtEx(RsTokenRemovePrivilegeDeniedByPrivilege,
      'RemoveIrrepealable', ClassName, RsUNToken, 0, false, [Privilege.Name]);
  ...
  SetLastError(0);
  if (not AdjustTokenPrivileges(Owner.TokenHandle, false, @privs, 0, nil, nil)) then
    raise EJwsclAdjustPrivilegeException.CreateFmtEx(RsPrivilegeCallAdjustTokenFailed, 'SetEnabled',
      ClassName, RsUNToken, 0, True, [Privilege.Name]);
...
begin
  ...
  if (not AdjustTokenPrivileges(Owner.TokenHandle, True, nil, 0, nil, nil)) then
    raise EJwsclAdjustPrivilegeException.CreateFmtEx(RsWinCallFailed, 'DisableAllPrivileges',
      ClassName, RsUNToken, 0, True, ['AdjustToken']);
...
begin
  ...
  Result := HeapAlloc(JwProcessHeap, HEAP_ZERO_MEMORY, sizeof(TLuidAndAttributes) * Count);
...
procedure TJwPrivilegeSet.Free_PLUID_AND_ATTRIBUTES(var Privileges: PLUID_AND_ATTRIBUTES);
begin
  if Privileges = nil then
    exit;

  if fPPrivilegesList.IndexOf(Privileges) < 0 then
    raise EJwsclSecurityException.CreateFmtEx(RsTokenInvalidPrivilegePointer,
      'Free_PLUID_AND_ATTRIBUTES', ClassName, RsUNToken, 0, false, []);
...
procedure TJwPrivilegeSet.Free_PTOKEN_PRIVILEGES(var Privileges: PTOKEN_PRIVILEGES);
begin
  if Privileges = nil then
    exit;
  { if fPPrivilegesList.IndexOf(Privileges) < 0 then
...
function TJwPrivilegeSet.Create_PTOKEN_PRIVILEGES: JwaWindows.PTOKEN_PRIVILEGES;
var
  Size, I: Integer;
...
begin
  ...
  Size := sizeof(JwaWindows.PTOKEN_PRIVILEGES) + sizeof(TLuidAndAttributes) * Count;
...
begin
  ...
  Result := HeapAlloc(JwProcessHeap, HEAP_ZERO_MEMORY, sizeof(JwaWindows.TPrivilegeSet) -
    sizeof(TLuidAndAttributes) + sizeof(TLuidAndAttributes) * Count);
...
procedure TJwPrivilegeSet.Free_PPRIVILEGE_SET(var Privileges: JwaWindows.PPRIVILEGE_SET);
begin
  if Privileges = nil then
    exit;
...
begin
  ...
  if I >= 0 then
    ...
    raise EJwsclPrivilegeNotFoundException.CreateFmtEx(RsTokenPrivlegeNotInList, 'DeletePrivilege',
...
begin
  if Assigned(Owner) then
    raise EJwsclNotImplementedException.CreateFmtEx(RsTokenPrivilegeAssignDenied, 'AddPrivilege',
...
begin
  ...
  if FAILED(hRes) then
    raise EJwsclPrivilegeNotFoundException.CreateFmtEx(RsTokenCallLookUpPrivilegeValueFailed,
      'AddPrivilege', ClassName, RsUNToken, 0, HRESULT_CODE(hRes), [PrivName]);
...
function TJwPrivilegeSet.AddPrivilege(LuidAttributes: LUID_AND_ATTRIBUTES): Integer;
var
  I: Integer;
begin
  if Assigned(Owner) then
  ...
  begin
    ...
    begin
      if (TJwPrivilege(fList.Items[I]).LUID.LowPart = LuidAttributes.LUID.LowPart) and
        (TJwPrivilege(fList.Items[I]).LUID.HighPart = LuidAttributes.LUID.HighPart) then
        raise EJwsclSecurityException.CreateFmtEx(RsTokenPrivilegeAlreadyInList, 'AddPrivilege',
          ClassName, RsUNToken, 0, false, [TJwPrivilege(fList.Items[I]).LUID.HighPart,
...
{$ENDIF}
    ...
    if (sName = nil) then
      raise EJwsclNotEnoughMemory.CreateFmtEx(RsTokenNotEnoughMemoryForPrivName, 'Create',
        ClassName, RsUNToken, 0, false, []);
...
{$ENDIF}
  ...
  function GetDisplayName(const aName: TJwString; out aLanguageID: cardinal): TJwString;
  var
    len: cardinal;
    sName: TJwPChar;
...
constructor TJwPrivilege.Create(anOwner: TJwPrivilegeSet; Name: String; Attributes: cardinal);
var
  L: TLuidAndAttributes;
begin
...
class function TJwPrivilege.MakeLUID_AND_ATTRIBUTES(const LowPart: cardinal; const HighPart: LONG;
...
class function TJwPrivilege.MakeLUID_AND_ATTRIBUTES(const LUID: TLuid; Attributes: cardinal)
  : TLuidAndAttributes;
begin
  Result.LUID := LUID;
  Result.Attributes := Attributes;
...
class function TJwPrivilege.MakeLUID(const LowPart: cardinal; const HighPart: LONG): TLuid;
begin
...
begin
  ...
  if FAILED(hRes) then
    raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'TextToLUID', ClassName,
      RsUNToken, 0, HRESULT_CODE(hRes), ['LookupPrivilegeValue']);
end;
...
begin
  ...
  Result := JwFormatString(RsPrivilegeLuidText, [aLUID.HighPart, aLUID.LowPart, I]);
end;
...
begin
  Result := '';
  if PrivilegeAttributes and SE_PRIVILEGE_ENABLED_BY_DEFAULT = SE_PRIVILEGE_ENABLED_BY_DEFAULT then
  begin
  ...
  if PrivilegeAttributes and SE_PRIVILEGE_USED_FOR_ACCESS = SE_PRIVILEGE_USED_FOR_ACCESS then
  begin
    if HumanReadable then
...
begin
  Result := JwFormatString(RsPrivilegeFormatText, [LUIDtoText(LUID), Name, DisplayName,
...
begin
  if not Assigned(Owner.Owner) then
    raise EJwsclNotImplementedException.CreateFmtEx(RsTokenNotAssignedPrivilege, 'GetEnabled',
      ClassName, RsUNToken, 0, false, []);
  ...
  Owner.Owner.CheckTokenAccessType(TOKEN_QUERY + TOKEN_ADJUST_PRIVILEGES,
    'TOKEN_QUERY,TOKEN_ADJUST_PRIVILEGES', 'TJwPrivilege.SetEnabled');
  ...
  *)
  if (not AdjustTokenPrivileges(Owner.Owner.TokenHandle, false, @privs, sizeof(privs), @prevState,
    @preLen)) then
  begin
    raise EJwsclAdjustPrivilegeException.CreateFmtEx(RsPrivilegeCallAdjustTokenFailed, 'GetEnabled',
  ...
  try
    ...
    begin
      if (not AdjustTokenPrivileges(Owner.Owner.TokenHandle, false, @prevState, preLen, nil, nil))
      then
      begin
        raise EJwsclAdjustPrivilegeException.CreateFmtEx(RsPrivilegeCallAdjustTokenFailed1,
          'GetEnabled', ClassName, RsUNToken, 0, True, [Self.Name]);
      end;
    end;
  finally
  end;
  ...
  Result := (Attributes and SE_PRIVILEGE_ENABLED = SE_PRIVILEGE_ENABLED) or
    (Attributes and SE_PRIVILEGE_ENABLED_BY_DEFAULT = SE_PRIVILEGE_ENABLED_BY_DEFAULT);
end;

function TJwPrivilege.IsEnabledByDefault: boolean;
begin
  Result := Attributes and SE_PRIVILEGE_ENABLED_BY_DEFAULT = SE_PRIVILEGE_ENABLED_BY_DEFAULT;
end;
...
begin
  if not Assigned(Owner.Owner) then
    raise EJwsclNotImplementedException.CreateFmtEx(RsTokenNotAssignedPrivilege, 'SetEnabled',
      ClassName, RsUNToken, 0, false, []);
  ...
  if En then
    if fPrivilege_Enabled_By_Default then
      ...
      privs.Privileges[0].Attributes := SE_PRIVILEGE_ENABLED or SE_PRIVILEGE_ENABLED_BY_DEFAULT
    else
      privs.Privileges[0].Attributes := SE_PRIVILEGE_ENABLED
  else
    privs.Privileges[0].Attributes := 0;

  if (not AdjustTokenPrivileges(Owner.Owner.TokenHandle, false, @privs, 0, nil, nil)) then
    raise EJwsclAdjustPrivilegeException.CreateFmtEx(RsPrivilegeCallAdjustTokenFailed, 'SetEnabled',
      ClassName, RsUNToken, 0, True, [Self.Name]);
...
begin
  ...
  if fTokenHandle = 0 then
    raise EJwsclInvalidTokenHandle.CreateFmtEx(RsTokenInvalidTokenHandle, sSourceProc, ClassName,
      RsUNToken, 0, false, []);
end;
...
begin
  ...
  try
    ...
    SD := TJwSecureGeneralObject.GetSecurityInfo(TokenHandle, // const aHandle: THandle;
    ...
    // ShowMessage('except');
    temp := TJwSecurityToken.CreateDuplicateExistingToken(TokenHandle, TOKEN_READ or TOKEN_QUERY or
      TOKEN_DUPLICATE);

    try
      SD := TJwSecureGeneralObject.GetSecurityInfo(temp.TokenHandle, // const aHandle: THandle;
        SE_KERNEL_OBJECT, // const aObjectType: TSeObjectType;
        [siOwnerSecurityInformation, siGroupSecurityInformation, siDaclSecurityInformation]
        // aSecurityInfo: TJwSecurityInformationFlagSet;
        );
  ...
  try
    ...
    Token.ConvertToImpersonatedToken(SecurityImpersonation, TOKEN_IMPERSONATE or TOKEN_DUPLICATE or
      TOKEN_READ or TOKEN_QUERY);
    try
      Result := TJwSecureGeneralObject.ConvertMaximumAllowed(SD, Token, TJwSecurityTokenMapping);
    finally
      FreeAndNil(Token);
    end;
  finally
...
constructor TJwSecurityToken.CreateTokenByProcess(const aProcessHandle: TJwProcessHandle;
  const aDesiredAccess: TJwAccessMask; const Duplicate: boolean = false);
var
...
begin
  ...
  if not bResult then
  begin
    raise EJwsclOpenProcessTokenException.CreateFmtEx(RsWinCallFailed, 'CreateTokenByProcess',
...
begin
  ...
  hProc := OpenProcess( { PROCESS_QUERY_INFORMATION or PROCESS_VM_READ, } PROCESS_ALL_ACCESS, false,
    ProcessID);
  if hProc = 0 then
    raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'CreateTokenByProcessId',
  ...
  try
    ...
    CreateTokenByProcess(hProc, DesiredAccess,
      (DesiredAccess and TOKEN_DUPLICATE = TOKEN_DUPLICATE));
...
constructor TJwSecurityToken.CreateTokenByThread(const aThreadHandle: TJwThreadHandle;
  const aDesiredAccess: TJwAccessMask; const anOpenAsSelf: boolean);
var
...
begin
  ...
  bResult := OpenThreadToken(hThread, aDesiredAccess, anOpenAsSelf, fTokenHandle);

  if not bResult then
  begin
    if GetLastError() = ERROR_NO_TOKEN then // no token available
    begin
      raise EJwsclNoThreadTokenAvailable.CreateFmtEx(RsTokenInvalidThreadToken,
        'CreateTokenByThread', ClassName, RsUNToken, 0, True, []);
    end
    else
    begin
      raise EJwsclOpenThreadTokenException.CreateFmtEx(RsTokenUnableToOpenThreadToken,
        'CreateTokenByThread', ClassName, RsUNToken, 0, True, []);
    end;
  end;
...
constructor TJwSecurityToken.CreateTokenEffective(const aDesiredAccess: TJwAccessMask);
var
  bResult: boolean;
...
begin
  ...
  bResult := OpenThreadToken(GetCurrentThread, aDesiredAccess, True, hTokenHandle);
  if bResult then
  begin
    Self.Create;
...
constructor TJwSecurityToken.Create(const aTokenHandle: TJwTokenHandle;
  const ShareTokenHandle: TJwSharedHandle; const aDesiredAccess: TJwAccessMask);
begin
  Self.Create;
...
constructor TJwSecurityToken.CreateCompatibilityQueryUserToken(const DesiredAccess: TJwAccessMask;
  const ProcessName: TJwString = ExplorerProcessName);
...
begin
  ...
  if ProcessID = 0 then
    raise EJwsclProcessNotFound.CreateFmtEx(RsProcessNotFound, 'CreateCompatibilityQueryUserToken',
      ClassName, RsUNToken, 0, false, [ProcessName]);
  ...
  if ProcessHandle = 0 then
    raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed,
      'CreateCompatibilityQueryUserToken', ClassName, RsUNToken, 0, True, ['OpenProcess']);

  try
    TokenOrig := TJwSecurityToken.CreateTokenByProcess(ProcessHandle, TOKEN_ALL_ACCESS);
...
begin
  if @_WtsGetActiveConsoleSessionID = nil then
    _WtsGetActiveConsoleSessionID := GetProcAddress(GetModuleHandle('kernel32.dll'),
...
constructor TJwSecurityToken.CreateWTSQueryUserTokenEx(const Server: TObject; SessionID: cardinal);
var
...
begin
  ...
  if (not TSRunning) then
    raise EJwsclTerminalServiceNecessary.CreateFmtEx(RsTokenUnsupportedWtsCall, 'Create', ClassName,
      RsUNToken, 0, false, []);

  if not JwIsPrivilegeSet(SE_TCB_NAME, pqt_Available) then
    raise EJwsclPrivilegeCheckException.CreateFmtEx(RsTokenPrivilegeNotHeld, 'Create', ClassName,
      RsUNToken, 0, false, [SE_TCB_NAME]);
  ...
  begin
    if not WinStationQueryUserToken(hServer, InternalWtsGetActiveConsoleSessionID, fTokenHandle)
    then
      if not WinStationQueryUserToken(hServer, WTS_CURRENT_SESSION, fTokenHandle) then
        raise EJwsclWinCallFailedException.CreateFmtEx(RsTokenCallWtsQueryUserTokenFailed,
          'WinStationQueryUserToken', ClassName, RsUNToken, 0, True, [SessionID]);
  end
  else
  begin
...
constructor TJwSecurityToken.CreateWTSQueryUserToken
  (SessionID: TJwSessionId = INVALID_HANDLE_VALUE);
begin
  RaiseOnInvalidPrimaryToken('CreateWTSQueryUserToken');

  if not(TJwWindowsVersion.IsWindowsXP(True) or TJwWindowsVersion.IsWindows2003(True)) then
    raise EJwsclUnsupportedWindowsVersionException.CreateFmtEx(RsTokenUnsupportedWtsCall, 'Create',
      ClassName, RsUNToken, 0, false, []);
  ...
  begin
    if not WTSQueryUserToken(InternalWtsGetActiveConsoleSessionID, fTokenHandle) then
      if not WTSQueryUserToken(WTS_CURRENT_SESSION, fTokenHandle) then
        raise EJwsclWinCallFailedException.CreateFmtEx(RsTokenCallWtsQueryUserTokenFailed,
          'WTSQueryUserToken', ClassName, RsUNToken, 0, True, [SessionID]);
  end
  else
  begin
...
constructor TJwSecurityToken.CreateDuplicateExistingToken(const aTokenHandle: TJwTokenHandle;
  const aDesiredAccess: TJwAccessMask; UseDuplicateExistingToken: boolean = false);
begin
  Self.Create;
  // Shared == true
...
constructor TJwSecurityToken.CreateDuplicateExistingToken(const Token: TJwSecurityToken;
...
constructor TJwSecurityToken.CreateRestrictedToken(PrevTokenHandle: TJwTokenHandle;
  const TokenAccessMask: TJwTokenAccessMask; const Flags: cardinal;
  const SidsToDisable: TJwSecurityIdList; const PrivilegesToDelete: TJwPrivilegeSet;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        raise EJwsclInvalidSecurityListException.CreateFmtEx(RsInvalidRestrictedSids,
  ...
  // [Hint] bRes := false;
  try
    bRes := JwaWindows.CreateRestrictedToken(PrevTokenHandle, Flags, cDisSids, pDisSids, cLuids,
      pLuids, cResSids, pResSids, fTokenHandle);
  finally
  ...
  if not bRes then
    raise EJwsclSecurityException.CreateFmtEx(RsWinCallFailed, 'CreateRestrictedToken', ClassName,
      RsUNToken, 0, True, ['CreateRestrictedToken']);
end;

procedure TJwSecurityToken.ConvertToImpersonatedToken(impLevel: SECURITY_IMPERSONATION_LEVEL;
...
begin
  ...
  CheckTokenAccessType(TOKEN_DUPLICATE + TOKEN_READ, 'TOKEN_DUPLICATE,TOKEN_READ',
  ...
  if DuplicateTokenEx(fTokenHandle, aDesiredAccess, nil, impLevel, TokenImpersonation,
    hNewTokenHandle) then
  begin
    ...
    raise EJwsclTokenImpersonationException.CreateFmtEx(RsTokenCallDuplicateTokenFailed1,
      'GetTokenInformation', ClassName, RsUNToken, 0, True, []);
end;

procedure TJwSecurityToken.ConvertToPrimaryToken(const aDesiredAccess: TJwAccessMask);
var
  hNewTokenHandle: TJwTokenHandle;
begin
  {
  ...
  // create a copy of the token
  if DuplicateTokenEx(fTokenHandle, aDesiredAccess, nil, DEFAULT_IMPERSONATION_LEVEL, TokenPrimary,
    hNewTokenHandle) then
  begin
    // we need to close the handle
    if not fShared then
    ...
    raise EJwsclTokenPrimaryException.CreateFmtEx(RsTokenCallDuplicateTokenFailed1,
      'GetTokenInformation', ClassName, RsUNToken, 0, True, []);
end;
...
begin
  ...
  if not JwaWindows.GetTokenInformation(hTokenHandle, TokenClass, ptrTokenType, 0, Result) then
  begin
    iError := GetLastError;
...
procedure TJwSecurityToken.GetTokenInformation(hTokenHandle: TJwTokenHandle;
  TokenInformationClass: TTokenInformationClass; out TokenInformation: Pointer);

  procedure doRaiseError(EClass: EJwsclExceptionClass; msg: TJwString);
  begin
    if EClass = nil then
      EClass := EJwsclTokenInformationException;
    raise EClass.CreateFmtEx(msg, 'GetTokenInformation', ClassName, RsUNToken, 0, True, []);
  end;
...
begin
  ...
  tokLen := Self.GetTokenInformationLength(hTokenHandle, TokenInformationClass);
  ...
  if (TokenInformation = nil) then // TODO: BUGBUG : On not enough Memory this will fail
    doRaiseError(EJwsclNotEnoughMemory, RsTokenNotEnoughMemoryTokenSave);
  ...
  Result := JwaWindows.GetTokenInformation(hTokenHandle,
    JwaWindows.TTokenInformationClass(TokenInformationClass), TokenInformation, tokLen, tokLen);

  if not Result then
  begin
    HeapFree(JwProcessHeap, 0, TokenInformation);
    doRaiseError(nil, RsTokenUnableGetTokenInformation);
    // TODO: BUG, duplicate use of same exception class for different errors
  end;
end;
...
procedure TJwSecurityToken.GetTokenSource(out SourceName: ShortString; out SourceLUID: TLuid);
var
  pSource: PTOKEN_SOURCE;
begin
  CheckTokenHandle('GetTokenSource');
...
begin
  ...
  if not Assigned(List) then
    raise EJwsclNILParameterException.CreateFmtEx(RsNilParameter, 'SetTokenGroups', ClassName,
      RsUNToken, 0, false, ['List']);
  ...
  try
    if not AdjustTokenGroups(fTokenHandle, // HANDLE TokenHandle,
      ...
      raise EJwsclWinCallFailedException.CreateFmtWinCall(RsWinCallFailed, 'SetTokenGroups',
        // sSourceProc
...
begin
  ...
  GetTokenInformation(fTokenHandle, JwaWindows.TokenRestrictedSids, Pointer(pGroups));
  try
    Result := TJwSecurityIdList.Create(True, pGroups);
  finally
    HeapFree(JwProcessHeap, 0, pGroups);
...
procedure TJwSecurityToken.SetTokenDefaultDacl(const aDefaultDCAL: TJwDAccessControlList);
// TOKEN_ADJUST_DEFAULT
var
  pDACL: TOKEN_DEFAULT_DACL;
begin
  CheckTokenHandle('SetfTokenDefaultDacl');
  CheckTokenAccessType(TOKEN_ADJUST_DEFAULT, 'TOKEN_ADJUST_DEFAULT', 'SetfTokenDefaultDacl');

  pDACL.DefaultDacl := aDefaultDCAL.Create_PACL;
  try

    if (not SetTokenInformation(fTokenHandle, JwaWindows.TokenDefaultDacl, Pointer(@pDACL),
      sizeof(TOKEN_DEFAULT_DACL))) then
      raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'SetTokenDefaultDacl',
        ClassName, RsUNToken, 0, True, ['SetTokenInformation']);
...
begin
  ...
  try
    ...
    if (not SetTokenInformation(fTokenHandle, JwaWindows.TokenOrigin, Pointer(@pOrigin),
      sizeof(pOrigin))) then
      raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'SetTokenOrigin', ClassName,
...
begin
  ...
  try
    ...
    if (not SetTokenInformation(fTokenHandle, JwaWindows.TokenOwner, Pointer(@pOwner),
      sizeof(pOwner))) then
      raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'SetTokenInformation',
        ClassName, RsUNToken, 0, True, ['SetTokenInformation']);
  finally
    // PopPrivileges;
  end;
...
begin
  ...
  try
    Result := TJwSecurityId.Create(PSidAndAttributes(@pPrimaryGroup^.PrimaryGroup));
  finally
...
begin
  ...
  if (not SetTokenInformation(fTokenHandle, JwaWindows.TokenPrimaryGroup, Pointer(@pPrimaryGroup),
    sizeof(pPrimaryGroup))) then
    raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'SetPrimaryGroup', ClassName,
      RsUNToken, 0, True, ['SetTokenInformation']);
end;
...
begin
  ...
  try
    ...
    if (not SetTokenInformation(fTokenHandle, JwaWindows.TokenSessionId, Pointer(@SessionID),
      sizeof(SessionID))) then
      raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'SetTokenSessionId',
        ClassName, RsUNToken, 0, True, ['SetTokenInformation']);
  finally
    // PopPrivileges;
  end;
...
begin
  ...
  begin
    privSet.Free;
    raise EJwsclPrivilegeNotFoundException.CreateFmtEx(RsTokenPrivilegeNotFound,
      'IsPrivilegeEnabled', ClassName, RsUNToken, 0, false, [Name]);
  end;
...
{$ENDIF VISTA}
begin
  TJwWindowsVersion.CheckWindowsVersion(cOsVista, True, 'GetRunElevation', ClassName, RsUNToken, 0);
...
{$IFDEF VISTA}
  ...
  // Raises an exception if errors occur
  Self.GetTokenInformation(fTokenHandle,
    JwaWindows.TTokenInformationClass(JwaWindows.TokenElevation), Pointer(privs));
...
{$ENDIF VISTA}
begin
  TJwWindowsVersion.CheckWindowsVersion(cOsVista, True, 'GetLinkedToken', ClassName, RsUNToken, 0);
...
{$IFDEF VISTA}
  ...
  try
    Result := TJwSecurityToken.Create(Data^.LinkedToken, shOwned, MAXIMUM_ALLOWED);
  finally
...
begin
  for Labels := low(TJwIntegrityLabelType) to high(TJwIntegrityLabelType) do
    if (Labels <> iltNone) and not Assigned(JwIntegrityLabelSID(Labels)) then
      raise EJwsclInitWellKnownException.CreateFmtEx(RsInitWellKnownNotCalled,
...
class procedure TJwSecurityToken.SaferComputeTokenFromLevel(const SaferLevelHandle
  : TSaferLevelHandle; const InAccessToken: TJwSecurityToken; SaferFlags: DWORD;
...
begin
  ...
  try
    ...
    if (not JwaWindows.SaferComputeTokenFromLevel(SaferLevelHandle, InAccessTokenInt.TokenHandle,
      @hToken, SaferFlags, @Result.CompareResult)) then
      raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'SaferComputeTokenFromLevel',
        ClassName, RsUNToken, 0, True, ['SaferComputeTokenFromLevel']);
    if hToken <> 0 then
    begin
...
var
  TokenResult: TJwSaferComputeTokenResult;
begin
...
begin
  if (LabelType <> iltNone) and not Assigned(JwIntegrityLabelSID(LabelType)) then
    raise EJwsclInitWellKnownException.CreateFmtEx(RsInitWellKnownNotCalled, 'SetIntegrityLevel',
...
begin
  if not Assigned(MandatorySid) then
    raise EJwsclNILParameterException.CreateFmtEx(RsNilParameter, 'SetIntegrityLevel',
      RsTokenGlobalClassName, RsUNToken, 0, false, ['MandatorySid']);
...
{$IFDEF VISTA}
  ...
  try
    if (not SetTokenInformation(fTokenHandle,
      JwaWindows.TTokenInformationClass(JwaWindows.TokenIntegrityLevel), Pointer(@mL), sizeof(mL)))
    then
      raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'SetIntegrityLevel',
        ClassName, RsUNToken, 0, True, ['SetTokenInformation']);
  finally
...
{$ENDIF VISTA}
begin
  TJwWindowsVersion.CheckWindowsVersion(cOsVista, True, 'GetIntegrityLevel', ClassName,
    RsUNToken, 0);
...
{$IFDEF VISTA}
  ...
  // Raises an exception if errors occur
  Self.GetTokenInformation(fTokenHandle,
    JwaWindows.TTokenInformationClass(JwaWindows.TokenIntegrityLevel), Pointer(mL));
...
{$ENDIF VISTA}
begin
  TJwWindowsVersion.CheckWindowsVersion(cOsVista, True, 'GetElevationType', ClassName,
    RsUNToken, 0);
...
{$IFDEF VISTA}
  ...
  // Raises an exception if errors occur
  Self.GetTokenInformation(fTokenHandle, JwaWindows.TTokenInformationClass(
...
{$ENDIF VISTA}
begin
  TJwWindowsVersion.CheckWindowsVersion(cOsVista, True, 'GetVirtualizationAllowed', ClassName,
    RsUNToken, 0);
...
{$IFDEF VISTA}
  ...
  // Raises an exception if errors occur
  Self.GetTokenInformation(fTokenHandle,
    JwaWindows.TTokenInformationClass(JwaWindows.TokenVirtualizationAllowed), Pointer(privs));
...
{$ENDIF VISTA}
begin
  TJwWindowsVersion.CheckWindowsVersion(cOsVista, True, 'GetVirtualizationEnabled', ClassName,
    RsUNToken, 0);
...
{$IFDEF VISTA}
  // Raises an exception if errors occur
  Self.GetTokenInformation(fTokenHandle, JwaWindows.TokenMandatoryPolicy, Pointer(p));
...
function TJwSecurityToken.CreateRestrictedToken(const TokenAccessMask: TJwTokenHandle;
  const Flags: cardinal; const SidsToDisable: TJwSecurityIdList;
  const PrivilegesToDelete: TJwPrivilegeSet; const RestrictedSids: TJwSecurityIdList)
  : TJwSecurityToken;
begin
  Result := TJwSecurityToken.CreateRestrictedToken(Self.TokenHandle, TokenAccessMask, Flags,
...
begin
  ...
  if not JwaWindows.DuplicateTokenEx(TokenHandle, AccessMask, LPSECURITY_ATTRIBUTES(Security),
    GetImpersonationLevel, GetTokenType, newTokenHandle) then
    EJwsclDuplicateTokenException.CreateFmtEx(RsWinCallFailed, 'CreateDuplicateToken', ClassName,
      RsUNToken, 0, True, ['DuplicateTokenEx']);
...
function TJwSecurityToken.CheckTokenMembership(aSidToCheck: TJwSecurityId): boolean;
var
  bRes: longbool;
begin
  ...
  bRes := True;
  if not JwaWindows.CheckTokenMembership(Self.TokenHandle, aSidToCheck.Sid, bRes) then
    raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'CheckTokenMembership',
...
var
  NtCompareTokens: function(FirstTokenHandle, SecondTokenHandle: THandle; var Equal: boolean)
    : NTSTATUS; stdcall;
  dRes: NTSTATUS;
begin
  ...
  try
    ...
    on Exception do
    begin
      raise EJwsclNotImplementedException.CreateFmtEx(RsTokenUnsupportedNTCompareTokens, 'IsEqual',
        ClassName, RsUNToken, 0, false, []);
    end;
  end;
  ...
  begin
    ...
    if not NT_SUCCESS(dRes) then
      raise EJwsclSecurityException.CreateFmtEx(RsTokenCallNTCompareTokensFailed, 'IsEqual',
        ClassName, RsUNToken, 0, True, [dRes]);
  end
  else
    raise EJwsclNotImplementedException.CreateFmtEx(RsTokenUnsupportedNTCompareTokens, 'IsEqual',
      ClassName, RsUNToken, 0, false, []);
...
class procedure TJwSecurityToken.RemoveThreadToken(const Thread: TJwThreadHandle);
var
...
begin
  ...
  if not JwaWindows.SetThreadToken(PHandle(pThread), 0) then
    raise EJwsclSecurityException.CreateFmtEx(RsTokenUnableRemoveToken, 'SetThreadToken', ClassName,
      RsUNToken, 0, True, []);
end;
...
begin
  ...
  if (TokenType <> TokenImpersonation) then
    raise EJwsclTokenPrimaryException.CreateFmtEx(RsTokeOnlyAttachImpersonatedToken,
  ...
  if not JwaWindows.SetThreadToken(PHandle(pThread), TokenHandle) then
    raise EJwsclSecurityException.CreateFmtEx(RsTokenFailedSetToken, 'SetThreadToken', ClassName,
      RsUNToken, 0, True, []);
end;
...
begin
  ...
  if TokenType = TokenImpersonation then
    CheckTokenAccessType(TOKEN_QUERY + TOKEN_IMPERSONATE, 'TOKEN_QUERY,TOKEN_IMPERSONATE',
      'ImpersonateLoggedOnUser')
  else
    CheckTokenAccessType(TOKEN_QUERY + TOKEN_DUPLICATE, 'TOKEN_QUERY,TOKEN_DUPLICATE',
      'ImpersonateLoggedOnUser');

  if not JwaWindows.ImpersonateLoggedOnUser(TokenHandle) then
    raise EJwsclSecurityException.CreateFmtEx(RsTokenFailedImpLoggedOnUser,
      'ImpersonateLoggedOnUser', ClassName, RsUNToken, 0, True, []);
end;

class function TJwSecurityToken.PrivilegeCheck(const ClientToken: TJwSecurityToken;
  const RequiredPrivileges: TJwPrivilegeSet; const RequiresAllPrivs: TJwPrivCheck): boolean;
var
  I: Integer;
  privs: TJwPrivilegeSet;
  Priv: TJwPrivilege;
begin
  if not Assigned(ClientToken) then
    raise EJwsclInvalidTokenHandle.CreateFmtEx(RsWinCallFailed, 'PrivilegeCheck', ClassName,
      RsUNToken, 0, false, ['ClientToken']);

  JwRaiseOnNilParameter(RequiredPrivileges, 'PrivilegeCheck', 'PrivilegeCheckEx', ClassName,
    RsUNToken);
...
function TJwSecurityToken.PrivilegeCheck(const RequiredPrivileges: TJwPrivilegeSet;
  const RequiresAllPrivs: TJwPrivCheck): boolean;
begin
  JwRaiseOnNilParameter(RequiredPrivileges, 'RequiredPrivileges', 'PrivilegeCheck', ClassName,
    RsUNToken);
...
function TJwSecurityToken.PrivilegeCheckEx(const RequiredPrivileges: TJwPrivilegeSet;
  const RequiresAllPrivs: TJwPrivCheck): boolean;
var
  pPriv: JwaWindows.PPRIVILEGE_SET;
  bRes: longbool;
begin
  JwRaiseOnNilParameter(RequiredPrivileges, 'RequiredPrivileges', 'PrivilegeCheckEx', ClassName,
    RsUNToken);
  ...
  try
    ...
    if not JwaWindows.PrivilegeCheck(TokenHandle, pPriv, bRes) then
      raise EJwsclSecurityException.CreateFmtEx(RsWinCallFailed, 'PrivilegeCheckEx', ClassName,
        RsUNToken, 0, True, ['PrivilegeCheck']);
...
class procedure TJwSecurityToken.PrivilegedServiceAuditAlarm(SubsystemName, ServiceName: TJwString;
  ClientToken: TJwSecurityToken; Privileges: TJwPrivilegeSet; AccessGranted: boolean);
...
begin
  bOldAuditPriv := false;
  if not Assigned(ClientToken) then
    raise EJwsclInvalidTokenHandle.CreateFmtEx(RsWinCallFailed, 'PrivilegedServiceAuditAlarm',
      ClassName, RsUNToken, 0, True, ['ClientToken']);
  ...
  primToken := TJwSecurityToken.CreateTokenByProcess(0, TOKEN_READ or TOKEN_QUERY or
    TOKEN_ADJUST_PRIVILEGES or TOKEN_AUDIT_SUCCESS_INCLUDE or TOKEN_AUDIT_SUCCESS_EXCLUDE or
...
{$ENDIF}
      (TJwPChar(SubsystemName), TJwPChar(ServiceName), ClientToken.TokenHandle, pPriv^,
      AccessGranted) then
    begin
      raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'PrivilegedServiceAuditAlarm',
...
procedure TJwSecurityToken.LoadUserProfile(var ProfileInfo: TJwProfileInfo;
  const ProfileMembers: TJwProfileMembers);

  function GetRoamingProfile(const UserName: TJwString): TJwString;
  var
    DomainControllerInfo:
{$IFDEF UNICODE}PDomainControllerInfoW{$ELSE}PDomainControllerInfoA{$ENDIF};
    Res: DWORD;
  ...
  begin
    ...
    try
      ...
      Res := {$IFDEF UNICODE}DsGetDcNameW{$ELSE}DsGetDcNameA{$ENDIF}
        (nil, nil, nil, nil, DS_BACKGROUND_ONLY or DS_RETURN_FLAT_NAME, DomainControllerInfo);
    except
    ...
    if NetUserGetInfo(PWideChar(WideString(intServerName)), // __in   LPCWSTR servername,
...
begin
  ...
  if not {$IFDEF UNICODE}LoadUserProfileW{$ELSE}LoadUserProfileA{$ENDIF}
    ...
    raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'LoadUserProfile', ClassName,
      RsUNToken, 0, True, ['LoadUserProfile']);
...
function TJwSecurityToken.GetCurrentUserRegKey(const DesiredAccess: TJwAccessMask): HKEY;
var
  Res: HRESULT;
begin
  ...
  if Res <> 0 then
    raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailedWithNTStatus,
      'RegOpenCurrentUser', ClassName, RsUNToken, 0, True, ['RegOpenCurrentUser', Res]);
end;

// instance function related to token context
class procedure TJwSecurityToken.ImpersonateAnonymousToken(const Thread: TJwThreadHandle);
begin
  if not JwaWindows.ImpersonateAnonymousToken(Thread) then
    raise EJwsclSecurityException.CreateFmtEx(RsTokenFailedImpAnonymousToken,
      'ImpersonateAnonymousToken', ClassName, RsUNToken, 0, True, []);
end;

class procedure TJwSecurityToken.ImpersonateSelf(const anImpersonationLevel
  : SECURITY_IMPERSONATION_LEVEL);
begin
  if not JwaWindows.ImpersonateSelf(anImpersonationLevel) then
    raise EJwsclSecurityException.CreateFmtEx(RsTokenFailedImpSelf, 'ImpersonateSelf', ClassName,
      RsUNToken, 0, True, []);
end;

class function TJwSecurityToken.LookupPrivilegeValue(const lpSystemName, lpName: TJwString;
  out lpLuid: TLuid; const lpDefaultLuid: TLuid; iTryCount: Integer): HRESULT;
var
  iRetries: Integer;
  bSuccess: boolean;
  aLUID: LUID;
...
{$IFDEF UNICODE}LookupPrivilegeValueW{$ELSE}LookupPrivilegeValueA{$ENDIF}
  ...
  begin
    ...
    if ((GetLastError() = ERROR_IO_PENDING) and ((aLUID.LowPart <> 0) and (aLUID.LowPart < $FF))
      // Make some assumptions about the low part since the value may be incorrect
      ) then
    begin
      // we got a LUID

      Result := HRESULT_FROM_WIN32(ERROR_NO_SUCH_PRIVILEGE) or not $80000000;
      // this is a SUCCESS hresult with error info
...
begin
  if not JwaWindows.RevertToSelf then
    raise EJwsclWinCallFailedException.CreateFmtEx(RsTokenFailedRevertSelf, 'RevertToSelf',
      ClassName, RsUNToken, 0, True, []);
end;

class procedure TJwSecurityToken.ImpersonateNamedPipeClient(hNamedPipe: THandle);
begin
  if not JwaWindows.ImpersonateNamedPipeClient(hNamedPipe) then
    raise EJwsclWinCallFailedException.CreateFmtEx(RsTokenFailedImpPipe,
...
begin
  ...
  if not Result then
    raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'HasThreadAToken', ClassName,
      RsUNToken, 0, True, ['OpenThreadToken']);

  if Handle <> INVALID_HANDLE_VALUE then
    CloseHandle(Handle);
end;

class function TJwSecurityToken.GetThreadToken(const aDesiredAccess: TJwAccessMask;
  const anOpenAsSelf: boolean): TJwSecurityToken;
begin
  try
    Result := TJwSecurityToken.CreateTokenByThread(0, aDesiredAccess, anOpenAsSelf);
  except
    Result := nil;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        raise EJwsclPrivilegeCheckException.CreateFmtEx(RsTokenPrivilegeNotFound,
          'CheckTokenPrivileges', ClassName, RsUNToken, 0, false, [Privileges[I]]);
      end;
    end;
  finally
    privSet.Free;
...
begin
  ...
  begin
    if GetLastError() <> 0 then
      raise EJwsclInvalidTokenHandle.CreateFmtEx(RsWinCallFailed, 'IsRestricted', ClassName,
        RsUNToken, 0, True, ['IsTokenRestricted']);
...
procedure TJwSecurityToken.CheckTokenAccessType(aDesiredAccessMask: TJwAccessMask;
  StringMask, SourceProc: TJwString);
...
begin
  if (Self.AccessMask and aDesiredAccessMask) <> aDesiredAccessMask then
    raise EJwsclAccessTypeException.CreateFmtEx(RsTokenCheckAccessTypeText, SourceProc, ClassName,
      RsUNToken, 0, false, [IntToBin(aDesiredAccessMask), StringMask, IntToBin(AccessMask),
      SourceProc, JwFormatAccessRights(AccessMask, TokenMapping),
...
begin
  Result := JwFormatString(RsTokenStatisticsText, [TJwPrivilege.LUIDtoText(fTokenId), // 0
...
constructor TJwSecurityToken.CreateLogonUser(sUsername, sDomain, sPassword: TJwString;
  dwLogonType, dwLogonProvider: cardinal);
...
{$IFDEF UNICODE}LogonUserW{$ELSE}
    LogonUserA
{$ENDIF}
    (TJwPChar(sUsername), TJwPChar(sDomain), TJwPChar(sPassword), dwLogonType, dwLogonProvider,
    fTokenHandle);

  if (not bResult) then
    raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'CreateLogonUser', ClassName,
      RsUNToken, 0, True, ['LogonUser']);
end;
...
begin
  ...
  try
    ...
    try
      ...
      if Res <> 0 then
        raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed, 'CreateNewToken', ClassName,
          RsUNToken, 0, True, ['ZwCreateToken']);
    finally
...
class function TJwSecurityToken.Create_OBJECT_ATTRIBUTES(const aRootDirectory: THandle;
  const anObjectName: TJwString; const anAttributes: cardinal;
  const aSecurityDescriptor: TJwSecurityDescriptor;
  const anImpersonationLevel: TSecurityImpersonationLevel;
  const aContextTrackingMode: SECURITY_CONTEXT_TRACKING_MODE; const anEffectiveOnly: boolean)
...
class procedure TJwSecurityToken.Free_OBJECT_ATTRIBUTES(anObjectAttributes: TObjectAttributes);
begin
  ...
  if (anObjectAttributes.SecurityDescriptor <> nil) then
    TJwSecurityDescriptor.Free_SD(PSECURITY_DESCRIPTOR(anObjectAttributes.SecurityDescriptor));
...
function TJwSecurityToken.GetSecurityDescriptor(const SecurityFlags: TJwSecurityInformationFlagSet)
  : TJwSecurityDescriptor;
begin
  CheckTokenHandle('GetSecurityDescriptor');
  CheckTokenAccessType(TOKEN_READ, 'TOKEN_READ', 'GetSecurityDescriptor');

  Result := TJwSecureGeneralObject.GetSecurityInfo(TokenHandle, SE_KERNEL_OBJECT, SecurityFlags);
end;

procedure TJwSecurityToken.SetSecurityDescriptor(const SecurityFlags: TJwSecurityInformationFlagSet;
  const SecurityDescriptor: TJwSecurityDescriptor);
begin
  CheckTokenHandle('SetSecurityDescriptor');
  CheckTokenAccessType(TOKEN_WRITE, 'TOKEN_WRITE', 'GetSecurityDescriptor');

  TJwSecureGeneralObject.SetSecurityInfo(TokenHandle, SE_KERNEL_OBJECT, SecurityFlags,
    SecurityDescriptor);
end;
  ...
  try
    ...
    try
      if not Sid.EqualSid(JwLocalSystemSID) then
        raise EJwsclInvalidPrimaryToken.CreateFmtEx(RsPrimaryTokenMustBeSystem, MethodName,
          ClassName, RsUNToken, 0, false, []);
    finally
      Sid.Free;
    end;
...
class function TJwSecurityToken.IsTokenLinkedForMappedDrives: boolean;
  ...
  begin
    ...
    if not TJwWindowsVersion.IsWindows2000(false) and TJwWindowsVersion.IsWindows64 then
    ...
    if Res <> { ERROR_SUCCESS } 0 then
      raise EJwsclWinCallFailedException.CreateFmtEx(RsWinCallFailed,
        'IsTokenLinkedForMappedDrives', ClassName, RsUNToken, 0, Res, [sErrSrc]);
  end;
...
begin
  case index of
    { primary } 1:
      Result := TokenType = TokenPrimary;
    { impersonation } 2:
      Result := TokenType = TokenImpersonation;
  else
    Result := false;
  end;
end;
...
begin
  ...
  CheckTokenAccessType(TOKEN_QUERY, 'TOKEN_QUERY', 'GetUserProfileDirectory');

  if not JwaWindows.
{$IFDEF UNICODE}GetUserProfileDirectoryW{$ELSE}GetUserProfileDirectoryA{$ENDIF UNICODE}
    (fTokenHandle, nil, dwLen) and (GetLastError() <> ERROR_INSUFFICIENT_BUFFER) then

    raise EJwsclWinCallFailedException.CreateFmtWinCall(RsWinCallFailed, // const sMsg: TJwString;
      'ExpandEnvironmentStrings', // sSourceProc,
      ClassName, // sSourceClass,
      RsUNToken, // sSourceFile: TJwString; iSourceLine:
      0, True, // Cardinal; bShowLastError: boolean;
...
function TJwSecurityToken.ExpandEnvironmentStrings(const EnvString: TJwString): TJwString;
var
  pEnv: Pointer;
...
begin
  ...
  if TJwWindowsVersion.IsWindows7(True) then
    ...
    CheckTokenAccessType(TOKEN_IMPERSONATE or TOKEN_QUERY, 'TOKEN_IMPERSONATE or TOKEN_QUERY',
  ...
  try
    ...
    if (SrcU.Length = 0) or (SrcU.Buffer = nil) then
      // RtlExpandEnvironmentStrings_U fails with AV if input string is empty
      exit;
    ...
    try
      ...
      if (DestU.Length = 0) or (DestU.Buffer = nil) then
        // RtlExpandEnvironmentStrings_U fails with AV if input string is empty
        exit;
...
begin
  ...
  pr := IsPrimaryToken;
  try
    Result := JwCreateToString([ClassName, '', 'Hash', GetHashCode, 'Handle', TokenHandle,
      'AccessMask', JwFormatAccessRightsSimple(AccessMask, TokenMapping), 'IsPrimaryToken', pr,
      'IsThreadtoken', IsThreadToken, 'IsRestricted', IsRestricted, 'SessionID', TokenSessionId,
      'PrimaryGroup', PrimaryGroup.ToString, 'TokenUserName', sUsername, 'UserName', UserName,
{$IFDEF JWSCL_TYPEINFO}
      'TokenType', GetEnumName(TypeInfo(TOKEN_TYPE), Integer(TokenType)),
{$ELSE}
      'TokenType', 'Unavailable value!! JWSCL_TYPEINFO is not defined.',
{$ENDIF JWSCL_TYPEINFO}
      'Shared', Shared, 'Origin', Format('%d.%d', [TokenOrigin.HighPart, TokenOrigin.LowPart]),
      'Groups', Groups.Count, 'GroupMembers', Groups.GetText(True), // TODO: instead .ToString
      'Restricted', Restricted.Count, 'RestrictedMembers', Restricted.GetText(True),
      // TODO: instead .ToString
      'Privileges', privs.Count, 'PrivilegesMembers', privs.GetText // TODO: instead .ToString
      ]);
...
function TJwSecurityToken.RetrieveSpecificAccessRights(const AccessMask: TJwAccessMask)
  : TJwAccessMask;
begin
  try
    if AccessMask = MAXIMUM_ALLOWED then
